...
auto fn(TaskScheduler &scheduler, Fn fn_task, TaskPriority priority,
  ...
  scheduler
      .schedule(Task{std::move(sched_fn),
                     stx::fn::rc::make_static(task_is_ready),
                     stx::fn::rc::make_static(no_next_schedule), priority,
                     std::move(trace_info)})
      .unwrap();
...
auto loop(TaskScheduler &scheduler, Loop<Fn> loop, TaskPriority priority,
  ...
  RcFn<void()> fn =
      stx::fn::rc::make_functor(scheduler.allocator, [state_ = stx::LoopState{},
                                                      loop_ = std::move(loop),
                                                      promise_ = std::move(
                                                          promise)]() mutable {
  ...
  scheduler
      .schedule(Task{std::move(fn), stx::fn::rc::make_static(task_is_ready),
                     stx::fn::rc::make_static(no_next_schedule), priority,
                     std::move(trace_info)})
      .unwrap();
...
auto forloop(TaskScheduler &scheduler, For<Fn> loop, TaskPriority priority,
  ...
  RcFn<void()> fn =
      stx::fn::rc::make_functor(scheduler.allocator, [state_ = stx::ForState{},
                                                      loop_ = std::move(loop),
                                                      promise_ = std::move(
                                                          promise)]() mutable {
        RequestProxy proxy{promise_};
