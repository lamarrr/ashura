...
struct DualSpinLock {
  ...
  STX_MAKE_PINNED(BiPartitionSpinLock)
...
struct GeneratorRingMemory final : private ManagerHandle {
  ...
  // destruction must be O(1) and be extremely short-lived.
  //
  void manager____deallocate() {
    ...
    if constexpr (!std::is_trivially_destructible_v<T>) {
      ...
      // this is based on the guarantee that the oldest element in the stream is
      // always destroyed first before the others.
      // and we use a `num_in_use` counter instead of the actual position of the
      ...
      // destroy chunk
      StreamChunk<T> *chunk = &memory_chunks[destroy_pos];
      chunk->~StreamChunk<T>();
