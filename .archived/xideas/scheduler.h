...
//
// NOTE: executors aren't thread-safe. don't submit tasks to them from the
// tasks.
//
//
//
...
struct DeferredTask {
  ...
  //
  //
  // i.e. once image is available on the File I/O executor, transfer to the GPU
  // executor
  //
  // hop
  //
  //
  // they need to maintain an Rc to the schedulers
  //
  //
  //
  // how the connect futures from here??? how will the user obtain the future??
  // Future<Future<Inputs>...>?
  //
  // can request cancel, we probably need to request a status
  //
  // (next(Future<Input>) => Option<Future<Output>>) => Future<Future<Output>>
  //
  // when none is returned, the scheduling task is assumed to have been canceled
  //
  // enum class NextStatus {  Cancel, Proceed    };
  //
  // we could reuse the task function but that would lead to more confusion as
  // to which to run and which not to
  //
  RcFn<void()> schedule;
  RcFn<TaskReady(nanoseconds)> poll_ready;
};
