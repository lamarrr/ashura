...
// TODO(lamarrr): spin lock will be confused for span lock
struct BiSpinLock {
  STX_MAKE_PINNED(BiSpinLock)

  BiSpinLock() {}

  SpinLock lock;
  bool first_locked : 1 = false;
  bool second_locked : 1 = false;
  ...
  void guard____lock_both() {
    CRITICAL_SECTION({
      ...
      do {
        ...
        got_access = !(first_locked || second_locked);
  ...
  void guard____lock_first() {
    CRITICAL_SECTION({
      bool got_access = false;

      do {
        LockGuard guard{lock};
        got_access = !first_locked;
        if (got_access) {
          first_locked = true;
        }
  ...
  void guard____lock_second() {
    CRITICAL_SECTION({
      bool got_access = false;

      do {
        LockGuard guard{lock};
        got_access = !second_locked;
        if (got_access) {
          second_locked = true;
        }
...
struct GeneratorRingMemory final : private ManagerHandle {
  ...
  // guarding 2 operations
  // 0 - allocation and its metadata
  // 1 - destruction of non-trivial objects
  BiSpinLock lock;
  union {
  ...
  void manager____unref() {
    ...
    CRITICAL_SECTION({
      // lock memory allocation and destruction of objects
      lock.guard____lock_both();
      ...
      // unlock only allocation, we need to run destructor on the
      // non-trivially destructible type.
      //
      // we could potentially only perform this special operation for
      // non-trivial types. but the relative perf benefit is not much within
      // this usage scope.
      //
      lock.guard____unlock_first();
    });
    ...
    // deletion request, it is suspended until the present one is done. but
    // allocation can continue until the buffer is exhausted without being
    // blocked whilst we are running the destructor.
    //
    //
