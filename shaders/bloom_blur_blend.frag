...
layout(set = 0, binding = 0) uniform Uniforms { float exposure; }
uniforms;
...
void main() {
  ...
  vec3 hdr_color = texture(scene_texture, in_st).rgb;
  vec3 bloom_color = texture(bloom_blur_texture, in_st).rgb;
