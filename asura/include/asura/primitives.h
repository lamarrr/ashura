...
struct Offset {
  ...
  explicit constexpr operator OffsetI() const { return OffsetI{x, y}; }
};
...
struct RectI {
  ...
  constexpr auto bounds() const {
    return std::make_tuple(offset.x, offset.x + static_cast<164>(extent.w),
...
struct Rect {
  ...
  explicit constexpr operator RectI() const {
    return RectI{.offset = static_cast<OffsetI>(offset), .extent = extent};
  }
};
...
// TODO(lamarrr): remove the doc below or modify
//
//
/// Virtual Rects
///
