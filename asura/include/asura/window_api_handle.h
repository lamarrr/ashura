...
#include "asura/ui/window_event_queue.h"
...
struct WindowApiHandle {
  ...
  ASR_MAKE_HANDLE(WindowApiHandle)
  ...
  WindowInfo get_window(WindowID id) {
    ...
    ASR_ENSURE(window_entry_pos != windows_info.end());
  ...
  void remove_window_info(WindowID id) {
    ...
    ASR_ENSURE(pos != windows_info.end());
      ...
      switch (event.type) {
        case SDL_WINDOWEVENT: {
          get_window(WindowID{event.window.windowID})
              .queue->add_raw(sdl_window_event_to_asr(event.window.event));
          return true;
        }
        ...
        case SDL_MOUSEBUTTONUP: {
          ...
          // SDL_StartTextInput()
          ASR_LOG("timestamp: {}", event.button.timestamp);
