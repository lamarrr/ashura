...
#include "asura/ui/pipeline.h"
#include "asura/ui/vk_render_context.h"
#include "asura/ui/vulkan.h"
...
void App::init() {
  ...
  sk_vk_context.fGetProc = [](char const* proc_name, VkInstance instance,
                              VkDevice device) {
    ASR_ENSURE(instance == nullptr || device == nullptr);
    ASR_ENSURE(!(instance != nullptr && device != nullptr));
    if (device != nullptr) {
  ...
  ASR_ENSURE(sk_direct_context != nullptr,
...
void App::tick() {
  ...
  if (backing_store_diff != BackingStoreDiff::None || window_extent_changed) {
    ...
    pipeline->tile_cache.update_dpr(
        dpr_from_extents(window->handle.handle->extent,
                         window->handle.handle->surface_extent));
