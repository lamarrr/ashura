...
#include "asura/ui/widgets/text.h"
...
#include "asura/subsystems/asset_loader.h"
...
inline std::unique_ptr<skia::textlayout::Paragraph> build_paragraph(
  ...
  ASR_ENSURE(paragraph_builder != nullptr);
...
void Text::tick(std::chrono::nanoseconds interval,
  ...
  auto tmp = context.get("ASR_AssetLoader").unwrap();
