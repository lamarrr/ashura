...
TEST(TileCacheTest, Basic) {
  ...
  EXPECT_EQ(cache.backing_store_offset, (IOffset{0, 0}));
  EXPECT_TRUE(cache.backing_store_offset_changed);
  EXPECT_EQ(cache.backing_store_extent, (Extent{1920, 1080}));
  ...
  EXPECT_EQ(cache.backing_store_offset, (IOffset{10, 10}));
  EXPECT_TRUE(cache.backing_store_offset_changed);
  EXPECT_EQ(cache.backing_store_extent, (Extent{2080, 1440}));
  EXPECT_TRUE(cache.backing_store_resized);
