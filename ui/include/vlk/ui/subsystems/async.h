...
enum class LockStatus { Unlocked, Locked };
...
enum class CompletionStatus : uint8_t { Pending, Completed };
...
struct FutureBaseState {
  ...
  VLK_DISABLE_COPY(FutureBaseState)
  ...
  void serve_cancelation_status(CancelationStatus status) {
    cancelation_status.value.store(status, std::memory_order_relaxed);
  }

  void serve_suspend_status(SuspendStatus status) {
    suspend_status.value.store(status, std::memory_order_relaxed);
  }

  void request_cancel() {
    CancelationStatus expected = CancelationStatus::Uncanceled;
    CancelationStatus target = CancelationStatus::CancelAcknowledged;
...
struct FutureState : public FutureBaseState {
  ...
  ~FutureState() {
    if (fetch_completion_status() == CompletionStatus::Completed) {
 ...
 private:
  T* laundered() { return std::launder(reinterpret_cast<T*>(&(info.storage))); }

  T const* laundered() const {
    return std::launder(reinterpret_cast<T const*>(&(info.storage)));
  }
};
...
struct Future {
  ...
  stx::Result<T, FutureError> copy() {
    return acquire_completion_state([this] { return state->unsafe_copy(); });
  }

  stx::Result<T, FutureError> move() {
    return acquire_completion_state([this] { return state->unsafe_move(); });
  }
...
struct CancelationToken {
  VLK_DEFAULT_CONSTRUCTOR(CancelationToken)

  template <typename T>
  explicit CancelationToken(Future<T> const& future)
      : state{std::static_pointer_cast<std::shared_ptr<FutureBaseState>>(
            future.state)} {}

  CancelationStatus fetch_status() const {
    return state->fetch_cancelation_status();
  }
...
struct CancelationProxy {
  VLK_DEFAULT_MOVE(CancelationProxy)
  VLK_DISABLE_COPY(CancelationProxy)

  template <typename T>
  explicit CancelationProxy(Future<T> const& future) {
    state = std::weak_ptr{
        std::static_pointer_cast<std::shared_ptr<FutureBaseState const>>(
            future.state)};
  }
...
struct SuspendToken {
  VLK_DEFAULT_CONSTRUCTOR(SuspendToken)

  template <typename T>
  explicit SuspendToken(Future<T> const& future)
      : state{std::static_pointer_cast<std::shared_ptr<FutureBaseState>>(
            future.state)} {}

  auto fetch_status() const { return state->fetch_suspend_status(); }

  void request_suspend() const { state->request_suspend(); }

  void request_resume() const { state->request_suspend(); }
...
struct SuspendProxy {
  VLK_DEFAULT_MOVE(SuspendProxy)
  VLK_DISABLE_COPY(SuspendProxy)

  template <typename T>
  explicit SuspendProxy(Future<T> const& future) {
    state = std::weak_ptr{
        std::static_pointer_cast<std::shared_ptr<FutureBaseState const>>(
            future.state)};
  }
...
struct Promise {
  ...
  void complete(T&& value) const {
    ...
    if (shared_state == nullptr) {
      ...
      shared_state->release_completion();
    }
  }
  ...
  PackagedTask(Task<T>&& task, Future<T> const& future)
      : function{[promise = Promise<T>(future),
          ...
          if (cancelation_proxy.try_acknowledge_cancel()) {
            ...
            promise.finish(function(cancelation_proxy));
          }
        }},
...
struct DefaultThreadExecutionContext {
  struct TaskQueueState {
    ...
    VLK_DISABLE_COPY(TaskQueueState)
