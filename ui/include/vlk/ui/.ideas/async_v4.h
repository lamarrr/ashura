...
// suspend =>
//
// if supend comes in and resume also comes in without the executor
// acknowledging either. the requests are both not executed.
//
// when submitting a request the bits of suspend and resume requests are
// meant to cancel each other. the submission operation for this queue is a
// bitwise AND.
//
//
// the queue is alway either having a suspend request or resume request or none
// TODO(lamarrr): how do we fetch and reset the queue???
//
//
//
// THIS DOESN'T SEEM CORRECT
//
enum class [[nodiscard]] SuspendRequests : uint8_t{
    // all suspend or resume requests have been attended to. new suspend or
    // resume events can now come in.
    Empty = 0b00,
    // one or more requests for suspension of the async operation has been
    // requested, if a resume request enters before the queue is marked as
    // empty, both requests are ignored
    SomeSuspend = 0b01,
    // one or more resume requests for resumption of the suspended async
    // operation has been requested.
    SomeResume = 0b10};

STX_DEFINE_ENUM_BIT_OPS(SuspendRequests)

enum class SuspendRequestedState : uint8_t { Resumed, Suspended };
...
struct FutureBaseState {
  ...
  CancelRequest try_begin_attending_to_cancel_request() {
    return cancel_request_queue.v.exchange(CancelRequest::Empty,
                                           std::memory_order_relaxed);
  }

  SuspendRequests try_begin_attending_to_suspension_request() {
    return static_cast<SuspendRequests>(suspend_request_queue.v.exchange(
        enum_uv(SuspendRequests::Empty), std::memory_order_relaxed));
  }
  ...
  void request_cancel() {
    cancel_request_queue.v.store(CancelRequest::Some,
                                 std::memory_order_relaxed);
  }
  ...
  void request_resume() {
    ...
    //
    suspend_request_queue.v.fetch_or(enum_uv(SuspendRequests::SomeResume),
                                     std::memory_order_relaxed);
    suspend_requested_state.
  }
  ...
  void request_suspend() {
    ...
    // they cancel each other.
    //
    suspend_request_queue.v.fetch_or(enum_uv(SuspendRequests::SomeSuspend),
                                     std::memory_order_relaxed);
  }
 ...
 private:
  ...
  CacheLineAligned<std::atomic<CancelRequest>> cancel_request_queue{
      CancelRequest::Empty};
  CacheLineAligned<SuspendRequestedState> suspend_requested_state{
...
struct RequestProxy {
  ...
  //
  //
  CancelRequest try_begin_attending_to_cancel_request() const {
    ...
    if (shared_state == nullptr) {
      ...
      return shared_state->try_begin_attending_to_cancel_request();
    }
  }

  SuspendRequest try_begin_attending_to_suspension_request() const {
    auto shared_state = state.lock();
    if (shared_state == nullptr) {
      // the task is canceled since the associated
      // future has been discarded
      return SuspendRequest::None;
    } else {
      return shared_state->try_begin_attending_to_suspension_request();
    }
  }
