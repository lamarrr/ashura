...
struct [[nodiscard]] Gradient{
    // linear, radial, sweep
};

struct [[nodiscard]] BoxAlignment {
...
struct [[nodiscard]] Border : public Edge {
  Color color;
...
using BorderRadius = Corner;

enum class [[nodiscard]] BoxLayout : uint8_t{Fit, ExpandVertical,
                                             ExpandHorizontal, Expand};

struct [[nodiscard]] BoxProperties {
  ...
  constexpr BoxProperties border(Color color, uint32_t x, uint32_t y)
      const noexcept {
  ...
  constexpr BoxProperties border_radius(BorderRadius const &value)
      const noexcept {
    BoxProperties out{*this};
  ...
  constexpr BoxProperties border_radius(uint32_t tl_br, uint32_t tr_bl)
      const noexcept {
...
struct [[nodiscard]] BoxDecoration {
  ...
  enum class [[nodiscard]] DrawOrder : uint8_t{None, ImageFirst, ColorFirst};
