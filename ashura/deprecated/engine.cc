...
#define TIMER_BEGIN(name)                                      \
  ::std::chrono::steady_clock::time_point name##_TIMER_Begin = \
      ::std::chrono::steady_clock::now()
#define TIMER_END(name, str)                                              \
  ASH_LOG_INFO(                                                           \
      FontRenderer, "Timer: {}, Task: {}, took: {}ms", #name, str,        \
      (::std::chrono::steady_clock::now() - name##_TIMER_Begin).count() / \
          1'000'000.0f)
...
Engine::Engine(AppConfig const &cfg, Widget *iroot_widget) :
    uuid_generator{
        stx::rc::make_inplace<PrngUuidGenerator>(stx::os_allocator).unwrap()},
    ...
    root_widget{iroot_widget}
{
  ...
  root_window = stx::Some(window_manager.create_window(
      cfg.name.c_str(), cfg.root_window_type, cfg.root_window_create_flags,
      cfg.root_window_extent));

  ASH_LOG_INFO(Init, "Creating root window");

  stx::Vec required_window_instance_extensions =
  ...
  for (vk::PhyDeviceInfo const &device : phy_devices)
  {
    ...
    ASH_LOG_INFO(Init, "Device(name: '{}', ID: {}, type: {})",
  ...
  stx::Rc<vk::PhyDeviceInfo *> phy_device =
      stx::rc::make(
          ...
          select_device(phy_devices, device_preference,
                        *root_window.value()->surface.value())
              .expect("Unable to find any suitable rendering device")[0]
              .copy())
          .unwrap();

  ASH_LOG_INFO(
      Init, "Selected Physical Device: Device(name: '{}', ID: {}, type: {})",
  ...
  renderer.init(
      xqueue->device->dev, xqueue->info.queue, xqueue->info.family.index,
  ...
  pipeline_manager.rebuild_for_renderpass(swp.render_pass,
                                          swp.msaa_sample_count);
  ...
  root_window.value()->on_mouse_click(
      stx::fn::rc::make_unique_static([](MouseClickEvent event) {
  ...
  root_window.value()->on_mouse_click(
      stx::fn::rc::make_unique_functor(stx::os_allocator, [this](MouseClickEvent
                                                                     event) {
  ...
  for (FontSpec const &spec : cfg.fonts)
  {
    ...
    ASH_LOG_INFO(Init, "Loading font: {} from file: {}", spec.name.view(),
                 spec.path.view());
    ...
    if (result.is_ok())
    {
      ...
      ASH_LOG_INFO(Init, "Loaded font: {} from file: {}", spec.name.view(),
                   spec.path.view());
      auto [atlas, image_buffers] = render_font_atlas(*result.value(), spec);
      for (usize i = 0; i < image_buffers.size(); i++)
      {
        atlas.bins[i].texture =
            render_resource_manager.add_image(image_buffers[i], false);
      }
      font_bundle
  ...
  for (Subsystem *subsystem : ctx.subsystems)
  {
    ...
    ASH_LOG_INFO(Context, "Initialized subsystem: {} (type: {})",
...
void Engine::tick(std::chrono::nanoseconds interval)
{
  ...
  do
  {
    ...
    {
      root_window.value()->recreate_swapchain(queue.value(),
    ...
    if (!root_window.value()->surface.value()->is_zero_sized_swapchain)
    {
      vk::SwapChain &swapchain =
          root_window.value()->surface.value()->swapchain.value();

      auto [state, swapchain_image_index] =
          root_window.value()->acquire_image();
      ...
      renderer.submit(
          swapchain.window_extent, swapchain.image_extent, swapchain.frame,
      ...
      swapchain_state = root_window.value()->present(queue.value()->info.queue,
