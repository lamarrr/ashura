...
void SdfEncoder::submit(GpuFramePlan plan)
{
    ...
    plan->add_pass([attachments = this->attachments_, texture_set = this->texture_set_,
                    world_to_ndc = this->world_to_ndc_, variant = this->variant_,
                    i_states, i_state_runs,
                    i_items](GpuFrame frame, gpu::CommandEncoder enc) {
        ...
        auto params = SdfPipelineParams{
          ...
          .params      = shader::SdfShaderParams{.world_to_ndc = world_to_ndc,
                                                 .items        = items.device_address()},
          .states      = states,
...
void QuadEncoder::submit(GpuFramePlan plan)
{
    ...
    plan->add_pass([attachments = this->attachments_, texture_set = this->texture_set_,
                    world_to_ndc = this->world_to_ndc_, variant = this->variant_,
                    i_states, i_state_runs,
                    i_quads](GpuFrame frame, gpu::CommandEncoder enc) {
        auto states     = frame->get<State>(i_states);
        auto state_runs = frame->get<u32>(i_state_runs);
        ...
        auto params = QuadPipelineParams{
          ...
          .textures    = frame->get(texture_set),
          .params      = shader::QuadShaderParams{.world_to_ndc = world_to_ndc,
                                                  .quads        = quads.device_address()},
          .states      = states,
          .state_runs  = state_runs,
          .variant     = variant
        };
...
void TriangleFillEncoder::submit(GpuFramePlan plan)
{
    ...
    plan->add_pass([attachments = this->attachments_, stencil_op = this->stencil_op_,
                    texture_set  = this->texture_set_,
                    world_to_ndc = this->world_to_ndc_, variant = this->variant_,
                    i_index_runs, i_states, i_state_runs, i_sets, i_vertices,
        ...
        auto params = TriangleFillPipelineParams{
          .framebuffer = framebuffer,
          .samplers    = sys.gpu->samplers(),
          .textures    = frame->get(texture_set),
          .params =
            shader::TriangleFillShaderParams{.world_to_ndc = world_to_ndc,
                                             .sets         = sets.device_address(),
                                             .vertices     = vertices.device_address(),
                                             .indices      = indices.device_address()},
...
void FillStencilEncoder::submit(GpuFramePlan plan)
{
    ...
    plan->add_pass([attachments  = this->attachments_,
                    world_to_ndc = this->world_to_ndc_, i_world_transforms, i_vertices,
                    i_indices, i_index_runs, i_states,
        ...
        auto params = FillStencilPipelineParams{
          ...
          .params =
            shader::FillStencilShaderParams{.world_to_ndc = world_to_ndc,
                                 .world_transforms =
                                              world_transforms.device_address(),
                                 .vertices = vertices.device_address(),
                                 .indices  = indices.device_address()},
          .index_runs = index_runs,
          .states     = states,
...
void BezierStencilEncoder::submit(GpuFramePlan plan)
{
    ...
    plan->add_pass([attachments  = this->attachments_,
                    world_to_ndc = this->world_to_ndc_, i_index_runs, i_states,
                    i_state_runs, i_items, i_vertices,
                    i_indices](GpuFrame frame, gpu::CommandEncoder enc) {
        auto index_runs = frame->get<u32>(i_index_runs);
        auto states     = frame->get<State>(i_states);
        auto state_runs = frame->get<u32>(i_state_runs);
        ...
        auto params = BezierStencilPipelineParams{
          ...
          .render_area        = {.offset = {}, .extent = image.extent().xy()},
          .params =
            shader::BezierStencilShaderParams{.world_to_ndc = world_to_ndc,
                                 .items        = items.device_address(),
                                 .vertices     = vertices.device_address(),
                                 .indices      = indices.device_address()},
          .index_runs = index_runs,
...
void VectorPathEncoder::submit(GpuFramePlan plan)
{
    ...
    plan->add_pass([attachments = this->attachments_, texture_set = this->texture_set_,
                    variant = this->variant_, world_to_ndc = this->world_to_ndc_,
                    i_index_runs, i_coverage_states, i_coverage_state_runs,
                    i_fill_states, i_fill_state_runs, i_vertices, i_indices,
                    i_coverage_items,
...

        {
            auto coverage_params = VectorPathCoveragePipelineParams{
              ...
              .params =
                shader::VectorPathCoverageShaderParams{
                                                       .cfg          = cfg,
                                                       .world_to_ndc = world_to_ndc,
                                                       .items        = coverage_items.device_address(),
                                                       .vertices     = vertices.device_address(),
                                                       .indices      = indices.device_address()},
              .index_runs = index_runs,
...

        {
            ...
            auto fill_params = VectorPathFillPipelineParams{
              ...
              .params =
                shader::VectorPathFillShaderParams{.cfg          = cfg,
                                                   .world_to_ndc = world_to_ndc,
                                                   .items =
                                                     fill_items.device_address()},
              .states     = fill_states,
...
void PbrEncoder::submit(GpuFramePlan plan)
{
    ...
    plan->add_pass([attachments = this->attachments_, stencil_op = stencil_op_,
        ...
        auto params = PBRPipelineParams{
          ...
          .samplers     = sys.gpu->samplers(),
          .textures     = frame->get(texture_set),
          .params       = shader::PbrShaderParams{.vertices = vertices.device_address(),
                                                  .indices  = indices.device_address(),
                                                  .items    = items.device_address(),
