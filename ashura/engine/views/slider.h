...
struct Slider : public View
{
  ...
  Frame         frame               = {.width};
  Frame         thumb_frame         = {.width};
  Frame         track_frame         = {.width};
  ...
  {
    Vec2 track_extent = track_frame(region.extent);
    ...
    if (levels != 0)
    {
      ...
      for (i32 i = 1; i < levels; i++)
      {
        ...
        canvas.rrect(ShapeDesc{
            .center = center,
            .extent = Vec2{min(1.0F, unit * region.extent.x), track_extent.y},
