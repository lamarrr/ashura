...
#include "ashura/engine/views/image.hpp"
...
ui::State Image::tick(Scope const &, Events const &, Fn<void(View &)>)
{
    state_.resolved.match(
      [&](None) {
          src_.match([&](None) { state_.resolved = Option<ash::ImageInfo>{none}; },
...
static void render_image(Canvas & canvas, CRect const & region, CRect const & clip,
...
void Image::render(Canvas & canvas, RenderInfo const & info)
{
    state_.resolved.match([&](None) {},
                          [&](Option<ash::ImageInfo> & opt) {
                              opt.match(
                                [&](ash::ImageInfo & img) {
                                    render_image(canvas, info.canvas_region, info.clip,
                                                 img, style_);
                                },
                                []() {});
                          },
