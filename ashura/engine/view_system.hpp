...
struct IViewSys
{
    ...
    /// packed sequentially. This only represents the parent node. Since the tree
    /// is rebuilt from scratch every time, the order is preserved in that parents
    ...
    /// @param active if the focus should be made an active focus, i.e. if it
    /// should be marked as an active focus. in some views, active focus may have
