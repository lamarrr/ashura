...
typedef Dict<gpu::SamplerInfo, Tuple<SamplerIndex, gpu::Sampler>, BitHash, BitEq, u32>
  SamplerCache;
...
struct [[nodiscard]] ColorImage
{
    static constexpr gpu::FormatFeatures FEATURES =
      gpu::FormatFeatures::ColorAttachment | gpu::FormatFeatures::ColorAttachmentBlend |
    ...
    static constexpr gpu::Format HDR_FORMATS[] = {gpu::Format::R16G16B16A16_SFLOAT};
...
struct [[nodiscard]] ColorMsaaImage
{
    ...
    /// preferably use `StoreOp::DontCare` and `LoadOp::Clear/LoadOp::DontCare` in
    /// the render passes.
    gpu::ImageView view = nullptr;
...
/// @brief Depth-stencil image for use as render target attachments
/// and transfer source/destination only.
struct [[nodiscard]] DepthStencilImage
{
    static constexpr gpu::FormatFeatures FEATURES =
      gpu::FormatFeatures::DepthStencilAttachment | gpu::FormatFeatures::SampledImage;
    ...
    static constexpr TextureIndex depth_sampled_texture_index = TextureIndex::Default;

    static constexpr TextureIndex depth_input_attachment_index = TextureIndex::Default;
...
struct [[nodiscard]] GpuBuffer
{
    ...
    gpu::DeviceAddress device_address = 0;
...
struct [[nodiscard]] GpuBufferSpan
{
    ...
    gpu::DeviceAddress device_address() const
    {
...
struct IGpuFramePlan
{
    ...
    void add_preframe_task(Lambda && task)
    {
        return add_preframe_task(
          dyn_lambda<GpuFrameTaskFn>(arena_, static_cast<Lambda &&>(task)).unwrap());
    }
...
struct TexelBufferUnion
{
    ...
    static constexpr gpu::Format ALL_FORMATS[] = {gpu::Format::R8_UNORM,
                                                  gpu::Format::R8_SNORM,
                                                  gpu::Format::R8_UINT,
                                                  gpu::Format::R8_SINT,
                                                  gpu::Format::R8G8B8A8_UNORM,
...
/// This object is prepared on the main/scheduling thread and submitted to the
/// GPU on a render thread. In order to ensure maximum execution overlap, we
/// defer as much work as possible to the render thread. We thus have copies of
/// some of the resources across the frames. The swapchain is also recreated and
/// acquired on the render thread. The primary color target might thus, be
/// out-of-sync with the swapchain extent, in that case, we blit from the target
...
struct IGpuFrame
{
    ...
    /// @brief The frame has finished executing on the GPU, run completion tasks
    /// or fetch data from GPU
...
/// @brief A GpuSys has two primary components: The GpuFramePlan and the
/// GpuFrame.
///
/// The GpuFramePlan describes the operations that need to be executed for a
/// specific frame. It also contains the description of all resources that will
/// need to be allocated/copied to the GPU in order to execute the frame.
///
///
/// The GpuFrame contains the GPU resources that are needed to actually execute
/// the frame.
///
/// The GpuFramePlan can be continously prepared whilst the frames are executing
/// on the render thread so the GPU is never starved of work. This also means we
/// can submit frames as soon as they are ready and immediately start doing
/// other work without any waits on the CPU.
///
/// To keep the queue saturated, we'd ideally want to have atleast N + 1
/// FramePlans and N+1 Frames ???????????
///
struct IGpuSys
{
    ...
    /// @brief Initialize the GPU system with the provided pipeline cache data and
    /// preference
    ///
    ...
    /// @brief Allocate a texture slot for the image and bind it to the textures
    /// descriptors at the start of the next frame
    TextureIndex alloc_texture_index(gpu::ImageView view);

    /// @brief Release a texture slot and unbind it from the textures descriptors
