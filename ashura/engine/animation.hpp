...
#include "ashura/std/lambda.hpp"
#include "ashura/std/math.hpp"
#include "ashura/std/range.hpp"
#include "ashura/std/super.hpp"
...
/// forcefully needed to be owned by the timeline. but only added for ease of
/// use.
///
///
...
struct Timeline
{
    ...
    Timeline & key_frame(Span<T> frames, Span<nanoseconds const> durations,
                         Span<Easing> easings)
    {
        ASH_CHECK(frames.size() == (durations.size() * 2), "");
        ASH_CHECK(durations.size() == easings.size(), "");
...
/// @param reversed_ to reverse the effect of the animation, i.e. move back in
/// time
...
struct Stagger
{
    ...
    /// @param width the dimension of the stagger pattern, i.e. the number of
    /// rows. Affects the pattern's granularity
    /// @param num_items the total number of items to be staggered, must be
    /// greater than item
...
/// @param row_weight weight controlling the relative influence of the row
/// position to the column position [0, 1]
/// @param reverse_row reverse the stagger direction in the row axis, i.e. the
/// elements at the end of the row will animate first
/// @param reverse_column reverse the stagger direction in the column axis, i.e.
/// the elements at the end of the column will animate first
