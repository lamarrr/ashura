...
void Window::attach_surface(stx::Rc<vk::Instance *> const &instance)
{
  ...
  this->surface = stx::Some(
      stx::rc::make_unique(stx::os_allocator, vk::Surface{.surface = surface, .instance = instance->instance}).unwrap());
}

void Window::recreate_swapchain(stx::Rc<vk::CommandQueue *> const &queue, u32 max_nframes_in_flight, spdlog::logger &logger)
{
  ...
  */
  ...
  VkPresentModeKHR preferred_present_modes[] = {VK_PRESENT_MODE_IMMEDIATE_KHR, VK_PRESENT_MODE_FIFO_RELAXED_KHR,
  ...
  surface.value()->change_swapchain(queue, max_nframes_in_flight, preferred_formats, preferred_present_modes,
                                    ...
                                    VkExtent2D{.width = window_extent.width, .height = window_extent.height}, msaa_sample_count,
                                    VK_COMPOSITE_ALPHA_OPAQUE_BIT_KHR, logger);
}
...
stx::Rc<Window *> create_window(stx::Rc<WindowApi *> api, WindowConfig cfg)
{
  ...
  SDL_Window *window = SDL_CreateWindow(cfg.title.c_str(), AS(i32, std::max<u32>(cfg.extent.width, 1)),
  ...
  cfg.min_extent.copy().match(
      [&](extent min_extent) { SDL_SetWindowMinimumSize(window, AS(i32, min_extent.width), AS(i32, min_extent.height)); },
      []() {});

  cfg.max_extent.copy().match(
      [&](extent max_extent) { SDL_SetWindowMaximumSize(window, AS(i32, max_extent.width), AS(i32, max_extent.height)); },
      []() {});
  ...
  stx::Rc<Window *> win = stx::rc::make_inplace<Window>(stx::os_allocator, std::move(api), window, WindowID{window_id},
                                                        cfg.extent, cfg.extent, std::move(cfg), std::this_thread::get_id())
                              .unwrap();
