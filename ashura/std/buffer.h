...
/// - The capacity of the memory is fixed and can't change; hence, it can't
/// shrink or increase its memory usage
...
struct [[nodiscard]] Buffer
{
    ...
    /// @brief return the total number of elements the buffer is capable of
    /// storing
    ...
    /// @brief try to remove `num` elements from the Buffer if there's up to `num`
    ...
    /// @brief Shift element `first` to the end of the buffer by `distance` and
    /// leave the shifted-from memory region uninitialized
    ...
    /// @brief extend the size of the Buffer by `extension` and leave the extended
    ...
    /// @brief resize the Buffer to size `new_size`. If the size is greater than
    /// the previous size, the extended region is left uninitialized.
...
struct [[nodiscard]] RingBuffer
{
    ...
    /// @returns true if there's enough memory to push an element onto the
    /// RingBuffer
    template <typename... Args>
