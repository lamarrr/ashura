...
/// @brief Fast user-space spinlock suitable for deterministic and short
/// critical sections. The spinlock is unpaced and can cause cache invalidation
/// and inefficient CPU usage, use with caution.
struct [[nodiscard]] ISpinLock
{
    ...
    void lock()
    {
        ...
        while (!flag.compare_exchange_weak(expected, target, std::memory_order_acquire,
                                           std::memory_order_relaxed))
        {
            expected = false;
            backoff_pause(poll);
...
struct [[nodiscard]] IRWSpinLock
{
    ...
    void lock_read()
    {
        ...
        while (true)
        {
            if (state.compare_exchange_weak(expected, target, std::memory_order_acquire,
                                            std::memory_order_relaxed))
            {
                return;
            }
...
struct [[nodiscard]] AtomicInit
{
    ...
    ~AtomicInit()
    {
        ...
        if (stage.load(std::memory_order_acquire) == FutureStage::Yielded) [[likely]]
        {
            v_.~T();
        }
    }
    ...
    [[nodiscard]] bool init(Args &&... args)
    {
        ...
        if (!stage.compare_exchange_strong(expected, target, std::memory_order_relaxed,
                                           std::memory_order_relaxed))
        {
            return false;
        }
...
struct [[nodiscard]] ISync
{
    ...
    template <typename... Args>
    constexpr ISync(Args &&... args) : lock_{}, data_{static_cast<Args &&>(args)...}
    {
    }
...
/// @brief A CPU Timeline TimelineSemaphore used for synchronization in
/// multi-stage cooperative multitasking jobs. Unlike typical Binary/Counting
/// Semaphores, A timeline semaphore is a monotonic counter representing the
...
/// - No deadlocks can occur when synchronization is done using this. This also
...
///
/// TimelineSemaphore should ideally not be destroyed before completion as there
/// could possibly be other tasks awaiting it.
///
...
struct [[nodiscard]] ITimelineSemaphore
{
    ...
    [[nodiscard]] bool complete(u64 complete_stage)
    {
        ...
        u64             current = 0;
        std::atomic_ref stage{stage_};
        while (!stage.compare_exchange_weak(current, next, std::memory_order_release,
    ...
    [[nodiscard]] u64 increment(u64 inc)
    {
        ...
        u64             target  = inc;
        std::atomic_ref stage{stage_};
        while (!stage.compare_exchange_weak(current, target, std::memory_order_release,
                                            std::memory_order_relaxed)) [[unlikely]]
        {
...
inline Result<RcTimelineSemaphore> semaphore(Allocator allocator, u64 initial_stage = 0)
{
...
Result<Stream<T>> stream(Inplace, Allocator allocator, u64 num_stages, Args &&... args)
{
...
struct [[nodiscard]] Future
{
    ...
    {
        return state_->ref().match([](T & v) -> Result<ref<T>> { return Ok{ref{v}}; },
...
struct [[nodiscard]] AnyFuture
{
    ...
    template <typename T>
    AnyFuture(Future<T> future) : state_{transmute(static_cast<Future<T> &&>(future))}
    {
    }
...
struct [[nodiscard]] TaskInfo
{
    ...
    /// @brief The task's main body and ticker. It handles task scheduling state
    /// flow. It also determines task readiness, repetition and continuation.
...
[[nodiscard]] inline bool poll_semaphores(Span<TimelineSemaphore const> semaphores,
...
{
    /// @brief thread-safe allocator to allocate tasks from, must be able to
    /// allocate page-sized allocations
    Allocator allocator;
...
struct IScheduler
{
    ...
    /// @param thread the index of the thread to schedule to. If none is
    /// specified, the task is scheduled to the main thread.
    ...
    {
        this->schedule(thread, TaskBody{static_cast<P &&>(poll),
    ...
    /// @brief Launch a one-shot task with multiple functions to be called in
    /// sequence
    ...
    auto flatten(Allocator future_allocator, Thread thread, FlattenFunc flatten_func,
                 Future<Result<Future<Result<T, E>>, E>> await)
    {
                ...
                switch (nesting_level)
                {
                    case 0:
                    {
                        ...
                        if (r0.is_err())
                        {
                            yield_fut
                              .yield(
                                flatten_func(Result<T, E>{Err{std::move(r0.err())}}))
                              .unwrap();
                            return false;
                        }
        ...
        this->schedule(thread,
                       Frame{.flatten_func{static_cast<FlattenFunc &&>(flatten_func)},
    ...
    requires (Same<CallResult<F, u64>, void> || Convertible<CallResult<F, u64>, bool>)
               ...
               u64 dim, P poll)
    {
        ...
        // we need to first dispatch a task that will poll for readiness, and once
        // the shard is ready for dispatch we dispatch the shards. we can avoid this
        ...
        // the dispatch as we don't expect the polling function to be thread-safe
