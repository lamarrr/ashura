...
/// @brief An allocated block of memory that can be aliased by multiple
/// resources.
struct IAlias
{
...
enum class HazardType : u8
{
    ...
    None            = 0,
    /// @brief Read hazards that need to be awaited. Subsequent reads are
    /// overlapped until a write occurs
    Reads           = 1,
    /// @brief Write hazard that needs to be awaited.
    Write           = 2,
    /// @brief Reads after a write that need to be awaited. Subsequent reads are
    /// allowed to overlap until a write occurs
    ReadsAfterWrite = 3
};
...
struct EncoderResourceStates
{
    ...
    CoreSparseVec<DescriptorSetId,
                  Vec<u32>,    // id-to-index map
                  Vec<u32>     // the last pass the descriptor set was accessed:
                               // initially U32_MAX
                  >
      descriptor_sets_;
