...
///
///
inline void generate_sdf_from_mono(u8 const *const src, u32 const src_pitch,
                                   u32 const width, u32 const height,
                                   u32 const sdf_spread, u8 *const output,
                                   u32 const output_pitch)
{
  for (i64 i = 0; i < height + sdf_spread * 2; i++)
  {
    for (i64 j = 0; j < width + sdf_spread * 2; j++)
    {
      ...
      {
        is_inside =
            (src[isrc * src_pitch + (jsrc / 8)] >> (7 - (jsrc % 8))) & 1U;
      }

      // the squared distance to the nearest neigbor that has a different
           ...
           ifield < std::min(isrc + sdf_spread + 1, (i64) height); ifield++)
      {
             ...
             jfield < std::min(jsrc + sdf_spread + 1, (i64) width); jfield++)
        {
          u8 const neighbor_is_inside =
              (src[ifield * src_pitch + (jfield / 8)] >> (7 - (jfield % 8))) &
              1U;
          if (neighbor_is_inside != is_inside) [[likely]]
          {
            i64 const neighbor_square_distance =
                (ifield - isrc) * (ifield - isrc) +
                (jfield - jsrc) * (jfield - jsrc);
            square_distance =
                std::min(square_distance, neighbor_square_distance);
          }
        }
      }

      i64 const distance =
          (u8) (127 * std::sqrt((f32) square_distance) / sdf_spread);
