...
// They need to be transformed into viewport coordinate space for stencil operations
// the enclosing rect as well
struct Path3d
{
  vec2 current_point;
  vec2 span;
  ...
  // how will this method handle intersection?
  ...
  void quadratic_bezier_to(vec2 cx, vec2 p1)
  {}
  void cubic_bezier_to(vec2 cx1, vec2 cx2, vec2 p1)
  {}
  void arc_to()
  {}
  void close()
  ...
  static void harfbuzz_move_to(hb_draw_funcs_t *dfuncs,
                               Path3d          *acc,
                               hb_draw_state_t *st,
  ...
  {
    acc->close();
    acc->move_to({to_x, to_y});
    fmt::println("move to: {}, {}", to_x, to_y);
  }

  static void harfbuzz_line_to(hb_draw_funcs_t *dfuncs,
                               Path3d          *acc,
                               hb_draw_state_t *st,
                               float to_x, float to_y,
                               void *user_data)
  {
    acc->line_to({to_x, to_y});
    fmt::println("line to: {}, {}", to_x, to_y);
  }

  static void harfbuzz_quadratic_to(hb_draw_funcs_t *dfuncs,
                                    Path3d          *acc,
                                    hb_draw_state_t *st,
                                    ...
                                    void *user_data)
  {
    acc->quadratic_bezier_to({control_x, control_y}, {to_x, to_y});
    fmt::println("quadractic to: c1={},{}     p2={}, {}", control_x, control_y, to_x, to_y);
  }

  static void harfbuzz_cubic_to(hb_draw_funcs_t *dfuncs,
                                Path3d          *acc,
                                hb_draw_state_t *st,
                                float control0_x, float control0_y,
                                float control1_x, float control1_y,
                                float to_x, float to_y,
                                void *user_data)
  {
    acc->cubic_bezier_to({control0_x, control0_y}, {control1_x, control1_y}, {to_x, to_y});
    fmt::println("cubic to: c1={},{}   c2={},{}   p3={}, {}", control0_x, control0_y, control1_x, control1_y, to_x, to_y);
  }

  static void draw_glyph(hb_font_t *font, Path3d *acc, u32 glyph)
  {
    // hb_face_get_glyph_count();
    hb_draw_funcs_t *dfuncs = hb_draw_funcs_create();
    hb_draw_funcs_set_move_to_func(dfuncs, (hb_draw_move_to_func_t) harfbuzz_move_to, NULL, NULL);
    hb_draw_funcs_set_line_to_func(dfuncs, (hb_draw_line_to_func_t) harfbuzz_line_to, NULL, NULL);
    hb_draw_funcs_set_quadratic_to_func(dfuncs, (hb_draw_quadratic_to_func_t) harfbuzz_quadratic_to, NULL, NULL);
    hb_draw_funcs_set_cubic_to_func(dfuncs, (hb_draw_cubic_to_func_t) harfbuzz_cubic_to, NULL, NULL);
