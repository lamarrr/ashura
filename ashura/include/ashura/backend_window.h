...
{
  BackendWindow(SDL_Window *iwindow) :
      window{iwindow}
  {}

  STX_MAKE_PINNED(BackendWindow)
  ...
  {
    ASH_SDL_CHECK(SDL_SetWindowSize(window, size.width, size.height) == 0);
  }
  ...
  {
    ASH_SDL_CHECK(SDL_SetWindowMinimumSize(window, min.width, min.height) == 0);
  }
  ...
  {
    ASH_SDL_CHECK(SDL_SetWindowMaximumSize(window, max.width, max.height) == 0);
  }
  ...
  void set_icon(ImageView image)
  {
    ...
    SDL_Surface *icon = SDL_CreateSurfaceFrom((void *) image.data.data(), image.extent.width, image.extent.height, image.extent.width * nchannel_bytes(image.format), fmt);
  ...
  void recreate_swapchain(stx::Rc<vk::CommandQueue *> const &queue, u32 max_nframes_in_flight, spdlog::logger &logger)        // TODO(lamarrr): use manual logger
  {
