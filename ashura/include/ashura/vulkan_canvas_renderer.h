...
{
  static constexpr VkQueryPipelineStatisticFlags PIPELINE_STATISTIC_QUERIES = VK_QUERY_PIPELINE_STATISTIC_INPUT_ASSEMBLY_VERTICES_BIT |
  ...
  void init(VkDevice adev, VkQueue aqueue, u32 aqueue_family_index, f32 atimestamp_period, VkPhysicalDeviceMemoryProperties const &amemory_properties, u32 amax_nframes_in_flight)
  {
    ...
    VkQueryPoolCreateInfo pipeline_statistics_query_pool_create_info{.sType              = VK_STRUCTURE_TYPE_QUERY_POOL_CREATE_INFO,
                                                                     .pNext              = nullptr,
                                                                     .flags              = 0,
    ...
    VkQueryPoolCreateInfo timestamp_query_pool_create_info{.sType              = VK_STRUCTURE_TYPE_QUERY_POOL_CREATE_INFO,
                                                           .pNext              = nullptr,
                                                           .flags              = 0,
    ...
    for (u32 i = 0; i < max_nframes_in_flight; i++)
    {
      ...
      ASH_VK_CHECK(vkCreateQueryPool(dev, &pipeline_statistics_query_pool_create_info, nullptr, &pipeline_statistics_query_pool));
      pipeline_statistics_query_pools.push_inplace(pipeline_statistics_query_pool).unwrap();

      VkQueryPool timestamp_query_pool;
      ASH_VK_CHECK(vkCreateQueryPool(dev, &timestamp_query_pool_create_info, nullptr, &timestamp_query_pool));
              ...
              FrameStats                       &frame_stats)
  {
    ...
    if (vkGetQueryPoolResults(dev, pipeline_statistics_query_pools[frame], 0, 1, sizeof(pipeline_statistics_query_query_results), pipeline_statistics_query_query_results, sizeof(u64), VK_QUERY_RESULT_64_BIT) == VK_SUCCESS)
    {
    ...
    if (vkGetQueryPoolResults(dev, pipeline_timestamp_query_pools[frame], 0, NPIPELINE_TIMESTAMP_QUERIES, sizeof(pipeline_timestamp_query_results), pipeline_timestamp_query_results, sizeof(u64), VK_QUERY_RESULT_64_BIT) == VK_SUCCESS)
    {
      frame_stats.gpu_time = nanoseconds{(nanoseconds::rep)(((f64) timestamp_period) * (f64) (pipeline_timestamp_query_results[1] - pipeline_timestamp_query_results[0]))};
    }
    ...
    vkCmdResetQueryPool(cmd_buffer, pipeline_statistics_query_pools[frame], 0, NPIPELINE_STATISTIC_QUERIES);
    vkCmdResetQueryPool(cmd_buffer, pipeline_timestamp_query_pools[frame], 0, NPIPELINE_TIMESTAMP_QUERIES);

    vkCmdBeginQuery(cmd_buffer, pipeline_statistics_query_pools[frame], 0, 0);
    vkCmdWriteTimestamp(cmd_buffer, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, pipeline_timestamp_query_pools[frame], 0);
    ...
    for (usize icmd = 0; icmd < cmds.size(); icmd++)
    {
      ...
      vkCmdDrawIndexed(cmd_buffer, cmd.nindices, cmd.ninstances, first_index, (i32) vertex_offset, cmd.first_instance);
    ...
    vkCmdWriteTimestamp(cmd_buffer, VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT, pipeline_timestamp_query_pools[frame], 1);
