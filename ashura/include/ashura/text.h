...
{
  std::string_view
      text;        // utf-8-encoded text, Span because string view doesnt support non-string types
  stx::Span<TextRun const>
      runs;        // parts of text not styled by a run will use the paragraphs run style
...
struct TextLayout
{
  ...
  static std::pair<stx::Span<hb_glyph_info_t const>, stx::Span<hb_glyph_position_t const>>
      shape_text_harfbuzz(Font const &font, f32 render_font_height, u32 not_found_glyph,
                          stx::Span<char const> text, hb_buffer_t *shaping_buffer,
                          ...
                          bool use_kerning, bool use_ligatures)
  {
    ...
    hb_buffer_set_replacement_codepoint(
        shaping_buffer,
        HB_BUFFER_REPLACEMENT_CODEPOINT_DEFAULT);        // invalid character replacement
  ...
  void layout(TextBlock const &block, f32 text_scale_factor,
              stx::Span<BundledFont const> const font_bundle, f32 const max_line_width)
  {
    lines.clear();
    run_segments.clear();
    ...
    hb_language_t const language_hb =
        block.language.empty() ?
            ...
            hb_language_from_string(block.language.data(), (int) block.language.size());
    ...
    {
      usize font =
          match_font(block.default_style.font, block.default_style.fallback_fonts, font_bundle);
      resolved_fonts.push(font < font_bundle.size() ? font : 0).unwrap();
    }
    ...
    for (SBUInteger paragraph_begin = 0; paragraph_begin < block.text.size();)
    {
      SBParagraphRef const sb_paragraph = SBAlgorithmCreateParagraph(
          ...
          block.direction == TextDirection::LeftToRight ? (SBLevel) SBLevelDefaultLTR :
                                                          (SBLevel) SBLevelDefaultRTL);
      ASH_CHECK(sb_paragraph != nullptr);
      SBUInteger const paragraph_length =
          SBParagraphGetLength(sb_paragraph);        // number of bytes of the paragraph
      SBLevel const paragraph_base_level =
          SBParagraphGetBaseLevel(sb_paragraph);        // base direction, 0 - LTR, 1 - RTL
      SBLevel const *const paragraph_levels = SBParagraphGetLevelsPtr(
          sb_paragraph);        // SheenBidi expands to byte level representation of the codepoints
      ...
      for (char const *run_text_begin = paragraph_text_begin; run_text_begin < paragraph_text_end;)
      {
        ...
        while (run_it < block.runs.end())
        {
          if (run_block_text_offset >= style_text_offset &&
              run_block_text_offset < (style_text_offset + run_it->size)) [[likely]]
          {
        ...
        TextStyle const &run_style =
            irun_style >= block.styles.size() ? block.default_style : block.styles[irun_style];
        ...
        for (char const *segment_text_begin = run_text_begin; segment_text_begin < run_text_end;)
        {
          ...
          stx::Span const segment_text{segment_text_begin,
                                       (usize) (segment_text_end - segment_text_begin)};

          auto const [glyph_infos, glyph_positions] = shape_text_harfbuzz(
              *font_bundle[run_font].font, font_bundle[run_font].atlas.font_height,
              font_bundle[run_font].atlas.space_glyph, segment_text, shaping_buffer, run_script_hb,
              run_direction_hb, language_hb, run_style.use_kerning, run_style.use_ligatures);
          ...
          for (usize i = 0; i < glyph_infos.size(); i++)
          {
            ...
            Vec2 const offset  = scale * Vec2{(f32) glyph_positions[i].x_offset / 64.0f,
           ...
           line_begin < paragraph_run_segments_end;)
      {
        ...
        for (TextRunSegment *direction_run_begin = line_begin; direction_run_begin < line_end;)
        {
          ...
          if (direction == TextDirection::RightToLeft)
          {
            ...
            stx::Span{direction_run_begin, (usize) (direction_run_end - direction_run_begin)}
                .reverse();
          }
               ...
               stx::Span{direction_run_begin, (usize) (direction_run_end - direction_run_begin)})
          {
            ...
            f32 const        ascent =
                text_scale_factor * font_bundle[run_segment.font].atlas.ascent * style.font_height;
            f32 const descent =
                text_scale_factor * font_bundle[run_segment.font].atlas.descent * style.font_height;
        ...
        lines
            .push_inplace(LineMetrics{
                ...
                .base_direction = (paragraph_base_level & 0x1) == 0 ? TextDirection::LeftToRight :
