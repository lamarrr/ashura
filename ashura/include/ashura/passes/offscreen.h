...
struct OffscreenPass
{
  ...
  struct State
  {
    ...
    DepthStencil         clear_depth_stencils[1] = {
        DepthStencil{.depth = 0, .stencil = 0}};
  } state;

  // bindings don't require changes to the resources, and can change for every
  // task execution these are input or output bindings
  struct Bindings
  {
  } bindings;
  ...
  void init(Graph &graph, CmdBuffer &cmd_buffer)
  {
    ...
    bool has_color =
        arguments.color_attachment_desc.format != Format::Undefined;
    bool has_depth_stencil =
        arguments.depth_stencil_attachment_desc.format != Format::Undefined;

    arguments.color_attachment_desc.usages |= ImageUsages::ColorAttachment;
    arguments.depth_stencil_attachment_desc.usages |=
        ImageUsages::DepthStencilAttachment;

    if (has_color)
    {
      resources.color_images[0] =
          graph.create_image(arguments.color_attachment_desc);
      resources.color_image_views[0] = graph.create_image_view(
          ImageViewDesc{.image       = resources.color_images[0],
    ...
    {
      resources.depth_stencil_images[0] =
          graph.create_image(arguments.depth_stencil_attachment_desc);
      resources.depth_stencil_image_views[0] =
          graph.create_image_view(ImageViewDesc{
              .image           = resources.depth_stencil_images[0],
    ...
    state.color_attachments[0] =
        RenderPassAttachment{.format   = arguments.color_attachment_desc.format,
                             .load_op  = arguments.color_load_op,
                             .store_op = arguments.color_store_op};
    state.depth_stencil_attachments[0] = RenderPassAttachment{
        .format   = arguments.depth_stencil_attachment_desc.format,
        .load_op  = arguments.depth_stencil_load_op,
        .store_op = arguments.depth_stencil_store_op};
    resources.render_pass = graph.create_render_pass(RenderPassDesc{
        .color_attachments = has_color ? stx::Span{state.color_attachments} :
                                         stx::Span<RenderPassAttachment>{},
        .depth_stencil_attachments =
            has_depth_stencil ? stx::Span{state.depth_stencil_attachments} :
                                stx::Span<RenderPassAttachment>{}});
    resources.framebuffer = graph.create_framebuffer(FramebufferDesc{
        .renderpass        = resources.render_pass,
        .color_attachments = has_color ?
                                 stx::Span{resources.color_image_views} :
                                 stx::Span<ImageView>{},
        .depth_stencil_attachments =
            has_depth_stencil ? stx::Span{resources.depth_stencil_image_views} :
                                stx::Span<ImageView>{}});
  }
  ...
  void execute(Graph &graph, CmdBuffer &cmd_buffer)
  {
    ...
    cmd_buffer.add(cmd::BeginRenderPass{
        .framebuffer = resources.framebuffer,
        .render_pass = resources.render_pass,
        .render_area = IRect{.offset = {0, 0},
                             .extent = arguments.color_attachment_desc.extent},
        .color_attachments_clear_values =
            has_color ? stx::Span{state.clear_colors} : stx::Span<Color>{},
        .depth_stencil_attachments_clear_values =
            has_depth_stencil ? stx::Span{state.clear_depth_stencils} :
                                stx::Span<DepthStencil>{}});
    cmd_buffer.add(cmd::DispatchTask{
        .index    = 0,
        .type     = PipelineType::Graphics,
        .bindings = {},
        .framebuffer =
            resources.framebuffer});        // TODO(lamarrr): what are we using
                                            // framebuffer for here?>
