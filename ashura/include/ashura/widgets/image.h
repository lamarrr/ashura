  ...
  virtual void draw(gfx::Canvas &canvas, rect area) override
  {
    switch (state)
    {
      ...
      case ImageState::Loaded:
      {
        ...
        {
          canvas.draw_image(image, area, s0, t0, s1, t1, colors::WHITE);
          return;
        }

        canvas.draw_rounded_image(image, area, rect{.offset = vec2{s0, t0}, .extent = vec2{s1 - s0, t1 - t0}}, props.border_radius, 360, colors::WHITE);
      }
      break;
