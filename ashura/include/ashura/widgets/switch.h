...
struct Switch : public Widget
{
  ...
  explicit Switch(Callback ion_changed = stx::fn::rc::make_unique_static(default_on_changed),
  ...
  virtual void draw(Context &ctx, gfx::Canvas &canvas) override
  {
    ...
    Tween color_tween = state ? Tween<Color>{props.inactive_track_color, props.active_track_color} :
                                Tween<Color>{props.active_track_color, props.inactive_track_color};
    Tween thumb_position_tween =
        state ? Tween<f32>{thumb_begin_x, thumb_end_x} : Tween<f32>{thumb_end_x, thumb_begin_x};
    EaseIn    curve;
    Color     color          = animation.animate(curve, color_tween);
    f32 const thumb_position = animation.animate(curve, thumb_position_tween);

    canvas.draw_round_rect_filled(area, Vec4::splat(props.height / 2), 90, color)
        .draw_circle_filled(area.offset + Vec2{thumb_position, 1.5f + thumb_radius}, thumb_radius,
                            180, props.thumb_color);
  }
